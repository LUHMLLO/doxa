...
func listAllDevices(db *sql.DB) {
	...
	res, err := db.Query("SELECT * FROM DemoTable")
	if err != nil {
		panic(err.Error())
	}
	defer res.Close()
...
func addNewDevice(db *sql.DB) {
	...
	insert, err := db.Query("INSERT INTO DemoTable (`DeviceName`, `TempSuperior`, `TempIntermedia`, `TempInferior`, `Usuario`) VALUES('DevClient - Smart Fridge #03', 9, 6, 3, 'goDev')")
	if err != nil {
		panic(err.Error())
	}
	defer insert.Close()
